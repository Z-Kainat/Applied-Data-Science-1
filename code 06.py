...
for i, year in enumerate(years_to_plot):
    ...
    plt.bar(np.arange(data.shape[0]) + (0.2 * i), data['fresh_water'],
...
for i, year in enumerate(years_to_plot):
    ...
    plt.bar(np.arange(data.shape[0]) + (0.2 * i), data['urban_population'],
            color=selected_colors[i], width=barWidth, label=str(year))
...
for country in countries:
    ...
    plt.plot(country_data['year'],
             country_data['rural_population'], label=country)
